  ...
  >
    <template #label>
      <label
        v-if="label"
        :class="
          computedDirection === 'horizontal' ? 'col-form-label' : 'form-label'
        "
        :for="id"
        >{{ label }}</label
      >
...
const computedSize = computed(() => {
  return bootstrapSize("form-control", props.size);
});
const computedDirection = computed(() => {
  return getBootValue(`direction.${props.direction}`);
});
const computedType = computed(() => {
  return getBootValue(`type.input.${props.type}`);
});
onBeforeMount(() => {
  if (props.playground) {
    emit("init", { size, direction, type: type_input });
  }
});
</script>
